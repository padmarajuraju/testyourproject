...
const VcCards = (props, datetopass) => {
  ...
  const getSlotAvailability = (queryParameter) => {
    ...
    let url = "http://3.90.253.77:8080/vaccination-center-service/api/v1/slots/available/" + queryParameter;
    return fetch(url,
      {
        headers: {
  ...
  const UserInformationByEmailId = (emailId) => {
    ...
    let url = "http://3.90.253.77:8080/user-service/api/v1/user/user/" + emailId;
    return fetch(url,
      {
        headers: {
          'Content-Type': 'application/json',
